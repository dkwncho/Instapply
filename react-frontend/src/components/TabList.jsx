...
const Tab = ({ tab, activeTab, onClick }) => (
  <li
    key={tab.key}
    className={`border-b-2 ${activeTab === tab.key ? "border-indigo-600 text-indigo-600" : "border-white text-gray-500"}`}
  >
    <button
      role="tab"
...
const TabList = ({ activeTab, setActiveTab }) => {
  return (
    <ul className="w-full mx-2 flex items-center gap-x-3 overflow-x-auto">
      {tabs.map((tab) => (
        <Tab key={tab.key} tab={tab} activeTab={activeTab} onClick={setActiveTab} />
      ))}
    </ul>
  );
};
